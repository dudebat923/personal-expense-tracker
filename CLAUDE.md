...
The Next.js application lives in the `personal-expense-tracker/` subdirectory. All commands below must be run from that directory.
...
> **Important:** Next.js 16.2.5 has breaking changes from earlier versions. Before writing any Next.js-specific code, check `personal-expense-tracker/node_modules/next/dist/docs/` for current APIs and conventions.
...
**Path alias:** `@/*` resolves to the project root (`personal-expense-tracker/`), so `import Foo from "@/components/Foo"` works from anywhere.
